...
**Managed tier coverage has progressed but is not yet complete.** File-based managed discovery now covers both macOS paths (`/Library/Application Support/ClaudeCode/` and `/etc/claude-code/`), including `managed-settings.json`, `CLAUDE.md`, `managed-mcp.json`, drop-in settings, and `rules/*.md` files. MDM/plist reading from the `com.anthropic.claudecode` preference domain is implemented. Security-scoped bookmark infrastructure exists for both managed roots. Remaining gaps: server-managed settings are not yet read, and the managed scope UI views do not yet expose the authorize/clear controls for `/etc/claude-code/` and `~/.claude.json`.
