...
final class ManagedSettingsLocatorTests: XCTestCase {
    ...
    func testLocateManagedClaudeMdFile_WhenFileExists_ReturnsURL() {
        let locator = ManagedSettingsLocator(
            fileSystem: MockManagedSettingsFileSystem(
                existingPaths: [
                    "/library/application support/claudecode/claude.md"
        ...
        XCTAssertEqual(urlPath(locator.locateManagedClaudeMdFile()), "/Library/Application Support/ClaudeCode/CLAUDE.md")
    }
...
final class ManagedScopeViewModelTests: XCTestCase {
    func testInspectorBuildsActiveFileBasedManagedScopeSummary() {
        let inspector = makeInspector(
            ...
            fileContentsByPath: [
                "/Library/Application Support/ClaudeCode/managed-settings.json": #"{"cleanupPeriodDays": 30}"#,
                "/Library/Application Support/ClaudeCode/managed-settings.d/10-team.json": #"{"cleanupPeriodDays": 45}"#,
                "/Library/Application Support/ClaudeCode/managed-mcp.json": #"{"mcpServers":{"filesystem":{"command":"managed-tool"}}}"#,
                "/Library/Application Support/ClaudeCode/CLAUDE.md": "# Managed policy\nUse approved tools only."
            ]
        )

        let viewModel = inspector.inspect()

        XCTAssertEqual(viewModel.status.detail, "File-based managed settings")
        XCTAssertEqual(viewModel.tierRows.first(where: { $0.title == "File-based managed settings" })?.statusStyle, .active)
        XCTAssertEqual(viewModel.managedMcp.serverIDs, ["filesystem"])
        XCTAssertEqual(viewModel.managedClaudeMd.statusStyle, .available)
        XCTAssertEqual(viewModel.managedClaudeMd.content, "# Managed policy\nUse approved tools only.")
        XCTAssertTrue(viewModel.fileSourceRows.contains(where: {
            $0.title == "CLAUDE.md" && $0.statusStyle == .available
        }))
    }

    func testInspectorMarksFileBasedTierSuppressedWhenMdmPolicyIsActive() {
        let inspector = makeInspector(
            existingPaths: [
                "/library/application support/claudecode",
                "/library/application support/claudecode/managed-settings.json"
            ...
            mdmResult: ParseResult(
                value: ["cleanupPeriodDays": .number(90)],
                issues: []
            )
        )
        ...
        XCTAssertEqual(viewModel.status.detail, "MDM / OS policy")
        XCTAssertEqual(viewModel.tierRows.first(where: { $0.title == "MDM / OS policy" })?.statusStyle, .active)
        XCTAssertEqual(viewModel.tierRows.first(where: { $0.title == "File-based managed settings" })?.statusStyle, .suppressed)
    }

    func testInspectorMakesManagedClaudeMdExplicitlyInaccessibleWhenSandboxRestricted() {
        let inspector = makeInspector(
            existingPaths: [],
            directoryPaths: [],
            fileContentsByPath: [:],
        ...
        XCTAssertEqual(viewModel.status.accessOutcome, .sandboxRestricted)
        XCTAssertEqual(viewModel.managedClaudeMd.statusStyle, .inaccessible)
        XCTAssertNil(viewModel.managedClaudeMd.content)
        XCTAssertTrue(viewModel.fileSourceRows.contains(where: {
            $0.title == "CLAUDE.md" && $0.statusStyle == .inaccessible
        }))
    }
    ...
    private func makeInspector(
        ...
        mdmResult: ParseResult<[String: JSONValue]> = ParseResult(value: [:], issues: []),
        ...
        let locator = ManagedSettingsLocator(
            fileSystem: MockManagedSettingsFileSystem(
                ...
                directoryContentsByPath: directoryContentsByPath,
                directoryErrorsByPath: [:]
            )
        )
        ...
        return ManagedScopeInspector(
            ...
            sandboxProbe: MockSandboxProbe(isSandboxed: sandboxed),
            mdmPolicyReader: MockMDMPolicyReader(result: mdmResult),
            dataLoader: { url in
                guard let content = fileContentsByPath[url.path] else {
                    throw NSError(domain: "ManagedScopeViewModelTests", code: 1, userInfo: [NSLocalizedDescriptionKey: "Missing file content for \(url.path)"])
                }
