...
final class SettingsParserTests: XCTestCase {
    ...
    func testParseValidHooksStructureFixture() throws {
        ...
        XCTAssertEqual(preToolUse.actions[2].template, "Review the pending tool call: $ARGUMENTS")
        XCTAssertEqual(preToolUse.actions[2].model, "claude-haiku-4-5-20251001")
        XCTAssertEqual(preToolUse.actions[3].handlerType, .agent)
        XCTAssertEqual(preToolUse.actions[3].agentId, "request-validator")
    ...
    func testParseMixedHookHandlerFixtureSupportsAllCurrentHandlerTypes() throws {
        ...
        XCTAssertEqual(event.actions[2].template, "Inspect the hook payload: $ARGUMENTS")
        XCTAssertEqual(event.actions[3].agentId, "delegator")
        XCTAssertEqual(codes, Set(expected.codes))
    }
    ...
    func testParseInvalidHookHandlerRequirementsFixtureReportsClearIssues() throws {
        ...
        XCTAssertTrue(result.issues.contains(where: { $0.code == .invalidHookShape && $0.keyPath == "hooks.PreToolUse[2].template" }))
        XCTAssertTrue(result.issues.contains(where: { $0.code == .invalidHookShape && $0.keyPath == "hooks.PreToolUse[3].agent_id" }))
    }

    // MARK: - Packet 05: Hook Parser Expansion Tests

    func testParseHookWithOnceShellTimeoutInSeconds() throws {
        let loader = FixtureLoader.shared
        let jsonString = try loader.loadString(
            familyPath: "parsers/settings",
            caseID: "hook_once_shell_timeout",
            section: "input",
            fileName: "settings.json"
        )
        let expected = try loader.loadExpectedIssueSet(
        ...
        let event = try XCTUnwrap(document.value.hooks?.events["PreToolUse"])
        let action = try XCTUnwrap(event.actions.first)
        XCTAssertEqual(action.handlerType, .command)
        XCTAssertEqual(action.command, "validate.sh")
        XCTAssertEqual(action.timeout, 30)
        XCTAssertEqual(action.once, true)
        XCTAssertEqual(action.shell, "bash")
    }

    func testParseHookHttpExtendedFields() throws {
        let loader = FixtureLoader.shared
        let jsonString = try loader.loadString(
            familyPath: "parsers/settings",
            caseID: "hook_http_extended",
            section: "input",
            fileName: "settings.json"
        )
        let expected = try loader.loadExpectedIssueSet(
        ...
        let event = try XCTUnwrap(document.value.hooks?.events["PostToolUse"])
        let action = try XCTUnwrap(event.actions.first)
        XCTAssertEqual(action.handlerType, .http)
        XCTAssertEqual(action.url, "https://hooks.example.com/notify")
        XCTAssertEqual(action.method, "POST")
        XCTAssertEqual(action.body, "{\"event\": \"$EVENT\", \"tool\": \"$TOOL_NAME\"}")
        XCTAssertEqual(action.headers?["Authorization"], "Bearer $TOKEN")
        XCTAssertEqual(action.headers?["Content-Type"], "application/json")
        XCTAssertEqual(action.allowedEnvVars ?? [], ["TOKEN"])
        XCTAssertEqual(action.timeout, 15)
    }

    func testParseHookPromptTypeWithTemplate() throws {
        let loader = FixtureLoader.shared
        let jsonString = try loader.loadString(
            familyPath: "parsers/settings",
            caseID: "hook_prompt_type",
            section: "input",
            fileName: "settings.json"
        )
        let expected = try loader.loadExpectedIssueSet(
        ...
        let event = try XCTUnwrap(document.value.hooks?.events["Stop"])
        let action = try XCTUnwrap(event.actions.first)
        XCTAssertEqual(action.handlerType, .prompt)
        XCTAssertEqual(action.template, "Summarise the changes made in this session: $ARGUMENTS")
        XCTAssertEqual(action.model, "claude-haiku-4-5-20251001")
        XCTAssertEqual(action.timeout, 45)
    }

    func testParseHookAgentTypeWithAgentId() throws {
        let loader = FixtureLoader.shared
        let jsonString = try loader.loadString(
            familyPath: "parsers/settings",
            caseID: "hook_agent_type",
            section: "input",
            fileName: "settings.json"
        )
        let expected = try loader.loadExpectedIssueSet(
        ...
        let event = try XCTUnwrap(document.value.hooks?.events["UserPromptSubmit"])
        let action = try XCTUnwrap(event.actions.first)
        XCTAssertEqual(action.handlerType, .agent)
        XCTAssertEqual(action.agentId, "request-screener")
        XCTAssertEqual(action.inputs?["policy"], .string("strict"))
        XCTAssertEqual(action.inputs?["max_tokens"], .number(2048))
        XCTAssertEqual(action.model, "claude-haiku-4-5-20251001")
        XCTAssertEqual(action.timeout, 60)
        XCTAssertEqual(action.once, true)
    }

    func testParseAttributionObjectFormatFixture() throws {
        let loader = FixtureLoader.shared
        let jsonString = try loader.loadString(
            familyPath: "parsers/settings",
            caseID: "attribution_object_format",
            section: "input",
            fileName: "settings.json"
        )
        let expected = try loader.loadExpectedIssueSet(
        ...
        XCTAssertFalse(result.hasErrors)
        XCTAssertEqual(Set(result.issues.map(\.code.rawValue)), Set(expected.codes))
        XCTAssertEqual(document.value.attribution?.commit, "Generated with Claude Code\n\nCo-Authored-By: Claude <noreply@anthropic.com>")
        XCTAssertEqual(document.value.attribution?.pr, "AI-assisted")
    }

    func testParseDeprecatedIncludeCoAuthoredByEmitsDeprecationIssue() throws {
        let loader = FixtureLoader.shared
        let jsonString = try loader.loadString(
            familyPath: "parsers/settings",
            caseID: "attribution_deprecated_key",
            section: "input",
            fileName: "settings.json"
        )
        let expected = try loader.loadExpectedIssueSet(
        ...
        XCTAssertFalse(result.hasErrors)
        XCTAssertEqual(Set(result.issues.map(\.code.rawValue)), Set(expected.codes))
        XCTAssertEqual(document.value.includeCoAuthoredBy, true)
        XCTAssertTrue(result.issues.contains(where: {
            $0.code == .deprecatedKey &&
            $0.severity == .info &&
            $0.keyPath == "includeCoAuthoredBy"
        }))
    }
    ...
    func testParseSandboxFullFixtureProducesNoIssues() throws {
        let loader = FixtureLoader.shared
        let jsonString = try loader.loadString(
            familyPath: "parsers/settings",
            caseID: "sandbox_full",
        ...
        XCTAssertEqual(sandbox.failIfUnavailable, true)
        XCTAssertEqual(sandbox.allowUnsandboxedCommands, false)
        XCTAssertEqual(sandbox.enableWeakerNetworkIsolation, false)
        XCTAssertEqual(network.allowedDomains ?? [], ["github.com", "registry.npmjs.org"])
        XCTAssertEqual(network.httpProxyPort, 8080)
        XCTAssertEqual(network.socksProxyPort, 1080)
    }

    func testParseSandboxInvalidPortFixtureEmitsInvalidValueIssues() throws {
        let loader = FixtureLoader.shared
        let jsonString = try loader.loadString(
            familyPath: "parsers/settings",
            caseID: "sandbox_invalid_port",
            section: "input",
            fileName: "settings.json"
        )
        ...
        let result = parser.parse(jsonString: jsonString, sourceURL: sourceURL)
        let document = try XCTUnwrap(result.value)
        let network = try XCTUnwrap(document.value.sandbox?.network)

        XCTAssertFalse(result.hasErrors)
        XCTAssertEqual(network.httpProxyPort, 99999)
        XCTAssertEqual(network.socksProxyPort, 0)
        XCTAssertTrue(result.issues.contains(where: {
            $0.code == .invalidValue &&
            $0.keyPath == "sandbox.network.httpProxyPort"
        }))
        XCTAssertTrue(result.issues.contains(where: {
            $0.code == .invalidValue &&
            $0.keyPath == "sandbox.network.socksProxyPort"
        }))
    }

    func testParseSandboxEmptyDomainFixtureEmitsInvalidValueIssue() throws {
        let loader = FixtureLoader.shared
        let jsonString = try loader.loadString(
            familyPath: "parsers/settings",
            caseID: "sandbox_empty_domain",
            section: "input",
            fileName: "settings.json"
        )
    ...
    static let validHooksStructure = """
    {
      "hooks": {
        "preToolUse": [
          ...
          {
            "type": "prompt",
            "template": "Review the pending tool call: $ARGUMENTS",
            "model": "claude-haiku-4-5-20251001"
          },
          {
            "type": "agent",
            "agent_id": "request-validator",
            "once": true
          }
        ],
        "postToolUse": {
