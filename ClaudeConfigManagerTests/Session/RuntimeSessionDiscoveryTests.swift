...
final class RuntimeSessionDiscoveryTests: XCTestCase {
    ...
    func testParseStatusLinePayloadReturnsErrorForInvalidJson() {
        let discovery = RuntimeSessionDiscovery()
        let result = discovery.parseStatusLinePayload(Data("not valid json".utf8))

        guard case .failure(let error) = result else {
            return XCTFail("Expected decoding failure for invalid JSON")
        }
        if case .invalidJsonFormat = error {
            // expected
        } else {
            XCTFail("Expected invalidJsonFormat error, got \(error)")
        }
    }

    func testParseStatusLinePayloadReturnsErrorForMissingModel() {
        let json = """
        {
          "cwd": "/tmp",
          "session_id": "sess-123",
        ...
        guard case .failure(let error) = result else {
            return XCTFail("Expected decoding failure for missing model")
        }
        XCTAssertEqual(error, .missingRequiredField(fieldName: "model"))
    }

    func testParseStatusLinePayloadReturnsErrorForMissingTranscriptPath() {
        let json = """
        {
          "cwd": "/tmp",
          "session_id": "sess-123",
        ...
        guard case .failure(let error) = result else {
            return XCTFail("Expected decoding failure for missing transcript_path")
        }
        XCTAssertEqual(error, .missingRequiredField(fieldName: "transcript_path"))
    }

    func testRuntimeSessionSnapshotCodableRoundTrip() throws {
        let snapshot = RuntimeSessionSnapshot(
            id: "sess-roundtrip",
            transcriptPath: "/tmp/sess.jsonl",
            ...
            usedPercentage: 0.75,
            fiveHourUsedPercentage: 10.5,
            fiveHourResetsAt: 1738425600,
            sevenDayUsedPercentage: 20.0,
            sevenDayResetsAt: 1738857600,
            capturedAt: Date(timeIntervalSince1970: 1738000000)
        )

        let encoded = try JSONEncoder().encode(snapshot)
        let decoded = try JSONDecoder().decode(RuntimeSessionSnapshot.self, from: encoded)

        XCTAssertEqual(decoded.id, snapshot.id)
        XCTAssertEqual(decoded.modelId, snapshot.modelId)
        XCTAssertEqual(decoded.totalCostUsd, snapshot.totalCostUsd)
        XCTAssertEqual(decoded.fiveHourResetsAt, snapshot.fiveHourResetsAt)
        XCTAssertEqual(decoded.capturedAt, snapshot.capturedAt)
    }

    func testRuntimeSessionSnapshotNotStaleWhenRecent() {
        let snapshot = RuntimeSessionSnapshot(
            id: "sess-fresh",
            transcriptPath: "/tmp/sess.jsonl",
            modelId: "claude-sonnet-4-6",
