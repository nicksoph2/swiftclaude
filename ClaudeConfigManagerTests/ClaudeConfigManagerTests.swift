...
final class FixtureLayoutContractTests: XCTestCase {
    func testFixtureContractContainsAllRequiredTopLevelFamilies() {
        ...
        let requiredFamilies = ["parsers", "resolvers", "validation", "session", "shared", "runtime"]
    ...
    func testResolverAndSessionFixtureExamplesArePresent() throws {
        ...
        let runtimeDescriptor = try loader.descriptor(familyPath: "runtime/session_snapshot", caseID: "valid_statusline")
        ...
        XCTAssertTrue(
            FileManager.default.fileExists(
                atPath: runtimeDescriptor.inputFileURL(named: "statusline.json").path
            )
        )
    }
