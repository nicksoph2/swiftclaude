...
struct ManagedScopeView: View {
    private let viewModel: ManagedScopeViewModel

    init(viewModel: ManagedScopeViewModel = ManagedScopeViewModel()) {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 18) {
                ...
                if viewModel.status.accessOutcome.requiresExplicitFallbackUI {
                    accessFallbackCard
                }
    ...
    private var status: ManagedScopeStatusModel {
        viewModel.status
    }
    ...
    private var accessFallbackMessage: String {
        switch status.accessOutcome {
        case .sandboxRestricted:
            return "This build cannot inspect /Library/Application Support/ClaudeCode/ from inside the macOS sandbox, so managed files are reported as inaccessible instead of being silently omitted."
        case .inaccessible:
    ...
    private var fileSourcesCard: some View {
        VStack(alignment: .leading, spacing: 14) {
    ...
    private var managedMcpCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                Text("Managed MCP")
                    .font(.headline)
    ...
    private var managedClaudeMdCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
