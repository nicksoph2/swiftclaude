        ...
        if let hooks = document.value.hooks {
            for event in hooks.events.values.sorted(by: { $0.eventName < $1.eventName }) {
                ...
                for (index, action) in event.actions.enumerated() {
                    ...
                    let hasPrompt = isNonEmpty(action.prompt)
                    ...
                    if let type = action.type?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased(), !type.isEmpty {
                        ...
                        } else if type == "prompt", !hasPrompt {
                            issues.append(
                                makeIssue(
                                    code: .schema("settings.hookActionTypeMismatch"),
                                    severity: .error,
                                    message: "Hook action type 'prompt' requires a prompt value.",
                                    context: context,
                                    keyPath: "\(actionPath).type"
                                )
                            )
                        } else if type == "agent", !hasPrompt {
                            issues.append(
                                makeIssue(
                                    code: .schema("settings.hookActionTypeMismatch"),
                                    severity: .error,
                                    message: "Hook action type 'agent' requires a prompt value.",
                                    context: context,
                                    keyPath: "\(actionPath).type"
                                )
                            )
                        ...
                        if !hasCommand, !hasURL, !hasPrompt {
                            issues.append(
                                makeIssue(
                                    code: .schema("settings.hookActionTransportShape"),
                                    severity: .error,
                                    message: "Hook action must define at least one of command, url, or prompt.",
                                    context: context,
                                    keyPath: actionPath
                                )
                            )
                    ...
                    if action.shell != nil || action.isAsync != nil {
                        ...
                        let isCommandType = effectiveType == "command" || (effectiveType.isEmpty && hasCommand && !hasURL && !hasPrompt)
                        if !isCommandType {
                            if action.shell != nil {
                                issues.append(
                                    makeIssue(
                    ...
                    if action.headers != nil || action.allowedEnvVars != nil {
                        let effectiveType = action.type?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() ?? ""
                        let isHTTPType = effectiveType == "http" || (effectiveType.isEmpty && hasURL && !hasCommand && !hasPrompt)
                        if !isHTTPType {
                    ...
                    if action.model != nil {
                        ...
                        let isPromptOrAgentType = effectiveType == "prompt" || effectiveType == "agent" ||
                            (effectiveType.isEmpty && hasPrompt && !hasCommand && !hasURL)
