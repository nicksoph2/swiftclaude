...
struct SettingsParser {
            ...
            guard prompt != nil else {
                issues.append(
                    SyntaxIssue(
                        code: .invalidHookShape,
                        severity: .error,
                        message: "Hook action type '\(handlerType.rawValue)' requires a 'prompt' string.",
                        sourcePath: sourcePath,
                        keyPath: "\(keyPath).prompt"
                    )
                )
                return
            }
    ...
    private func validateRegistryCoveredKeys(
        ...
        for (key, value) in rawTopLevel.sorted(by: { $0.key < $1.key }) {
            guard let definition = registry.matchingDefinition(forTopLevelKey: key) else {
                continue
            }
